...
export default function ProviderPage() {
  ...
  const handleReport = async () => {
    if (reported) return;
    const reason = prompt('Kya problem hai? (optional)');
    try {
      await axios.post(api.report(slug), { reason: reason || '' });
      setReported(true);
      alert('Report submit ho gaya ✅ Team review karegi.');
      ...
      <div className="max-w-lg mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
          ...
          <div className="px-6 pb-6 space-y-3">
            {provider.showPhone === false && !showPhone ? (
              <div className="space-y-3">
                <div className="bg-orange-50 rounded-xl border border-orange-200 p-4 text-center">
                  <p className="text-sm text-orange-700 font-medium mb-2">🔒 Phone number hidden</p>
                  <p className="text-xs text-orange-500 mb-3">Provider ne phone number chhupaya hai</p>
              ...
              <>
                ...
                <a href={`https://wa.me/91${provider.phone}?text=Hi ${encodeURIComponent(provider.businessName || provider.name)}, I need your service`} target="_blank" rel="noopener noreferrer"
            ...
            <a href={`https://wa.me/?text=${encodeURIComponent('Check out ' + (provider.businessName || provider.name) + ' on NewSetu: ' + window.location.href)}`} target="_blank" rel="noopener noreferrer"
              className="flex items-center justify-center gap-2 w-full py-3 bg-green-50 hover:bg-green-100 text-green-700 font-semibold rounded-xl transition text-sm border border-green-200">
          ...
          <div className="px-6 pb-6 flex justify-center">
            <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm text-center">
              ...
              <p className="text-xs text-gray-400 mt-2">Scan karke profile dekhein</p>
            </div>
          </div>
        </div>
        ...
        <div className="text-center mt-6 space-y-2">
          ...
          <button onClick={handleReport} className="text-xs text-red-300 hover:text-red-500 transition">
            {reported ? '✅ Reported' : '🚨 Report this profile'}
          </button>
        </div>
